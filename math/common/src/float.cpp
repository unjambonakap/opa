...
std::unique_ptr<Float> internal_pi;
std::unique_ptr<Float> internal_e;

MpfrGState Float::g_params;
static bool is_init = false;

void Float::set_precision(int prec) {
  Float::g_params.precision = prec;
  internal_pi.reset(new Float(Float(-1).acos()));
  internal_e.reset(new Float(Float(1).exp()));
}

void Float_init(int seed) {
  if (!is_init) {
    is_init = true;
    Float::set_precision(FLAGS_opa_float_precision);
...
void Float::from_uli(unsigned long int v) { mpfr_set_ui(m_state, v, MPFR_RNDN); }
...
void Float::from_Float(const Float &peer) { mpfr_set(m_state, peer.m_state, MPFR_RNDN); }
...
bool Float::eq(const Float &peer) const { return mpfr_equal_p(m_state, peer.m_state); }
bool Float::diff(const Float &peer) const { return mpfr_lessgreater_p(m_state, peer.m_state); }
bool Float::lt(const Float &peer) const { return mpfr_less_p(m_state, peer.m_state); }
bool Float::gt(const Float &peer) const { return mpfr_greater_p(m_state, peer.m_state); }
bool Float::le(const Float &peer) const { return mpfr_lessequal_p(m_state, peer.m_state); }
bool Float::ge(const Float &peer) const { return mpfr_greaterequal_p(m_state, peer.m_state); }

Float Float::sqrt() const { DO_MPFR_OP(mpfr_sqrt(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::cos() const { DO_MPFR_OP(mpfr_cos(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::sin() const { DO_MPFR_OP(mpfr_sin(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::tan() const { DO_MPFR_OP(mpfr_tan(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::acos() const { DO_MPFR_OP(mpfr_acos(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::asin() const { DO_MPFR_OP(mpfr_acos(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::atan() const { DO_MPFR_OP(mpfr_acos(res.m_state, m_state, MPFR_RNDN), res); }
...
Float Float::abs() const { DO_MPFR_OP(mpfr_abs(res.m_state, m_state, MPFR_RNDN), res); }

bool Float::eq(const Float &peer, const Float &eps) const { return sub(peer).abs() < eps; }

Float Float::pow(const Float &pw) const {
  DO_MPFR_OP(mpfr_pow(res.m_state, m_state, pw.m_state, MPFR_RNDN), res);
}

Float Float::neg() const { DO_MPFR_OP(mpfr_neg(res.m_state, m_state, MPFR_RNDN), res); }
...
Float Float::exp() const { DO_MPFR_OP(mpfr_exp(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::log_e() const { DO_MPFR_OP(mpfr_log(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::log_2() const { DO_MPFR_OP(mpfr_log2(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::log_10() const { DO_MPFR_OP(mpfr_log10(res.m_state, m_state, MPFR_RNDN), res); }
Float Float::log_base(const Float &base) const { return log_e() / base.log_e(); }
Float Float::atanh() const { DO_MPFR_OP(mpfr_atanh(res.m_state, m_state, MPFR_RNDN), res); }

Float Float::tanh() const { DO_MPFR_OP(mpfr_tanh(res.m_state, m_state, MPFR_RNDN), res); }

std::complex<Float> Float::iexp() const { return std::complex<Float>(cos(), sin()); }

Float Float::ceil() const { DO_MPFR_OP(mpfr_ceil(res.m_state, m_state), res); }
Float Float::floor() const { DO_MPFR_OP(mpfr_floor(res.m_state, m_state), res); }
Float Float::round() const { DO_MPFR_OP(mpfr_round(res.m_state, m_state), res); }

std::string Float::to_data() const {
  std::ostringstream ss;
  ss << m_state->_mpfr_exp << m_state->_mpfr_prec << m_state->_mpfr_sign;
  ss << std::string_view((const char *)m_state->_mpfr_d, mpfr_custom_get_size(m_state->_mpfr_prec));
  return ss.str();
}
...
template <> u32 FloatUtil::cast<Float, u32>(const Float &from) { return from.to_bignum().getu32(); }
template <> bignum FloatUtil::cast<Float, bignum>(const Float &from) { return from.to_bignum(); }
template <> u32 FloatUtil::cast<double, u32>(const double &from) {
  return Float(from).to_bignum().getu32();
}
template <> bignum FloatUtil::cast<double, bignum>(const double &from) { return u32(from); }
