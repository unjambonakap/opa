...
public:
  template <class T>
  static void from_str(const opa::stolen::StringRef &a, T &out) {
    out = T(a);
  }

  template <class T>
  static T from_str2(const std::string &a) {
    T out;
...
class Conv2 {
public:
  template <class From, class To> static To conv(const From &x) {
    return To(x);
  }
};
...
template <class T> std::string get_bin_str(const T &v) {
  std::string res;
  REP (i, 8 * sizeof(T))
    res += '0' + GETB_BLOCK(&v, i);
  return res;
}

template <class T> std::string dump_hex(const T &v) {
  return b2h((const u8 *)&v, sizeof(v));
}
...
void _Join(const opa::stolen::StringRef &ref, std::ostringstream &os,
...
template <class T>
std::string Join(const opa::stolen::StringRef &ref, const std::vector<T> &tb) {
  std::ostringstream os;
  REP (i, tb.size()) {
    if (i) os << ref.data();
    os << Conv::to_str(tb[i]);
...
public:
  template <class T>
  static void fill1(const opa::stolen::StringRef &ref, T &out) {
    Conv::from_str(ref, out);
  }

  static void fill(const std::vector<opa::stolen::StringRef> &tb,
                   std::tuple<Args...> &out) {
    fill1(tb[N], std::get<N>(out));
    TemplateIterator<N - 1, Args...>::fill(tb, out);
  }

  static void tuple_to_string_vec(const std::tuple<Args...> &a,
...
template <typename... Args>
std::tuple<Args...>
string_vec_to_tuple(const std::vector<opa::stolen::StringRef> &tb) {
  std::tuple<Args...> res;
...
std::vector<std::string> tuple_to_string_vec(const std::tuple<Args...> &a) {
  ...
  internal::TemplateIterator<sizeof...(Args)-1, Args...>::tuple_to_string_vec(
    a, res);
  return res;
}
