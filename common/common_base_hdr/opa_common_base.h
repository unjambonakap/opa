...
#define GETB_BLOCK(a, b) (BIT_BLOCK(a, b) >> ((b) & (sizeof((a)[0]) * 8 - 1)) & 1)
...
#define LIST2VEC(lst) (std::vector<__typeof(lst[0])>(lst, lst + LIST_COUNT(lst)))
...
#define OPA_DECL_COUT_OPERATOR3(CL, ...)                                                           \
  std::string str() const { return OPA_STREAM_STR(RAW_OPA_DISP_VARS(__VA_ARGS__)); }               \
...
#define _OPA_NAMESPACE_END3(a, b, c) _OPA_NAMESPACE_END1(a) _OPA_NAMESPACE_END2(b, c)

#define RAW_OPA_NAMESPACE(...) OPA_DISPATCH(_OPA_NAMESPACE, __VA_ARGS__)(__VA_ARGS__)

#define RAW_OPA_NAMESPACE_END(...) OPA_DISPATCH(_OPA_NAMESPACE_END, __VA_ARGS__)(__VA_ARGS__)
...
#define _OPA_DISP_VAR5(a, b, c, d, e) _OPA_DISP_VAR4(a, b, c, d) << _OPA_DISP_VAR1(e)
#define _OPA_DISP_VAR6(a, b, c, d, e, f) _OPA_DISP_VAR5(a, b, c, d, e) << _OPA_DISP_VAR1(f)
#define _OPA_DISP_VAR7(a, b, c, d, e, f, g) _OPA_DISP_VAR6(a, b, c, d, e, f) << _OPA_DISP_VAR1(g)
...
#define OPA_DISP_VARSERR(...) (std::cerr << RAW_OPA_DISP_VARS(__VA_ARGS__) << std::endl)
...
#define OPA_DISPL0(...)                                                                            \
  (std::cout << OPA_LINE_INFO << ": " << RAW_OPA_DISP_VARS(__VA_ARGS__) << std::endl)
#define OPA_DISPL(msg, ...)                                                                        \
  (std::cout << OPA_LINE_INFO << "msg:" << msg << ": " << RAW_OPA_DISP_VARS(__VA_ARGS__)           \
             << std::endl)

#define OPA_STREAM_STR(a) (((std::ostringstream() << a)).str())

#define OPA_DISP_VARS1(a) (std::cout << _OPA_DISP_VAR1(a) << std::endl)
#define OPA_DISP_VARS2(a, b) (std::cout << _OPA_DISP_VAR2(a, b) << std::endl)
#define OPA_DISP_VARS3(a, b, c) (std::cout << _OPA_DISP_VAR3(a, b, c) << std::endl)
#define OPA_DISP_VARS4(a, b, c, d) (std::cout << _OPA_DISP_VAR4(a, b, c, d) << std::endl)
#define OPA_DISP_VARS5(a, b, c, d, e) (std::cout << _OPA_DISP_VAR5(a, b, c, d, e) << std::endl)
...
#define OPA_DECL_SPTR_TMPL(cl, cl2) template <class T> using cl2 = std::shared_ptr<cl<T> >;
...
#define OPA_DECL_SPTR_TMPL3(cl, cl2)                                                               \
  template <class A, class B, class C> using cl2 = std::shared_ptr<cl<A, B, C> >;
...
#define _OPA_FIELDS7(x, a, b, c, d, e, f, g) x.a, _OPA_FIELDS6(x, b, c, d, e, f, g)

#define RAW_OPA_FIELDS(x, ...) OPA_DISPATCH(_OPA_FIELDS, __VA_ARGS__)(x, __VA_ARGS__)
...
#define RAW_OPA_LT_OP(x, ...) OPA_DISPATCH(_OPA_LT_OP, __VA_ARGS__)(x, __VA_ARGS__)
...
#define RAW_OPA_EQ_OP(x, ...) OPA_DISPATCH(_OPA_EQ_OP, __VA_ARGS__)(x, __VA_ARGS__)
...
#define RAW_OPA_COPY_OP(x, ...) OPA_DISPATCH(_OPA_COPY_OP, __VA_ARGS__)(x, __VA_ARGS__)
...
#define RAW_OPA_CONSTRUCTOR_OP(...) OPA_DISPATCH(_OPA_CONSTRUCTOR_OP, __VA_ARGS__)(__VA_ARGS__)

#define OPA_CONSTRUCTOR_OP(cl, body, ...)                                                          \
  cl(TYPE_AND_NAME_TO_ARGS(__VA_ARGS__)) : RAW_OPA_CONSTRUCTOR_OP(__VA_ARGS__) { body; }
...
#define OPA_TGEN_DECL_BASE(typ, ...)                                                               \
  ...
  static void tgen_load(ARGS_JOIN(::, __VA_ARGS__)::typ &, const google::protobuf::Any &);         \
  static void tgen_store(const ARGS_JOIN(::, __VA_ARGS__)::typ &, google::protobuf::Any &);        \
...
static std::ostream &operator<<(std::ostream &os, const s128 &a) {
  u128 b;
  if (a < 0) {
    os << "-";
    b = -a;
...
std::ostream &operator<<(std::ostream &os, const std::unordered_map<A, B, L> &a) {
  os << "(";
  for (auto &x : a) os << x << ",";
  os << ")";
  return os;
...
public:
  static std::ostream &os_pipe_tuple(std::ostream &os, const std::tuple<Args...> &a) {
...
template <class T> std::ostream &operator<<(std::ostream &os, const std::initializer_list<T> &a) {
  os << "(";
  for (auto &x : a) os << x << ",";
  os << ")";
  return os;
...
template <class T, class L> std::ostream &operator<<(std::ostream &os, const std::set<T, L> &a) {
  os << "(";
  for (auto &x : a) os << x << ",";
  os << ")";
  return os;
...
template <class T> std::ostream &operator<<(std::ostream &os, const std::deque<T> &a) {
  os << "(";
...
static inline u64 count_bit_slow(u64 n) { return (n == 0) ? 0 : 1 + count_bit_slow(n & (n - 1)); }
...
template <class T> std::shared_ptr<T> make_sptr(T *a) { return std::shared_ptr<T>(a); }
#endif
...
template <class T> using SmallPQ = std::priority_queue<T, std::vector<T>, std::greater<T> >;
