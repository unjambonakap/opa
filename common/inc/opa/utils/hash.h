...
template <typename A, typename B> struct hash<std::pair<A, B> > {
  size_t operator()(const std::pair<A, B> &v) const { return absl::HashOf(v.first, v.second); }
};
...
template <class Tuple, size_t Index = std::tuple_size<Tuple>::value - 1> struct _hash_helper {
...
template <class Tuple, size_t Index = std::tuple_size<Tuple>::value - 1> struct _eq_helper {
...
template <typename Subclass, typename ReturnValue, typename Context, typename... Args>
class MemoizeHelper {
public:
  ...
  const ReturnValue &get(const FuncType &func, const Args &...args) const {
    ...
    if (res.second) {
      ...
      *cur = ((const Subclass *)this)->template real_get<FuncType>(func, args...);
      return *cur;
    }
  ...
  mutable std::unordered_map<TupleInput, ReturnValue *, ::opa::utils::hash_utils::hash<Args...> >
    store;
};

template <typename ReturnValue, typename Context, typename... Args>
class MemoizeHelperFunc : public MemoizeHelper<MemoizeHelperFunc<ReturnValue, Context, Args...>,
...
template <typename ReturnValue, typename Context, typename... Args>
class MemoizeHelperClass : public MemoizeHelper<MemoizeHelperClass<ReturnValue, Context, Args...>,
                                                ReturnValue, Context, Args...> {
public:
  template <typename Method> ReturnValue real_get(const Method &method, const Args &...args) const {
    return (this->context->*method)(args...);
  }
};

#define CALL_mem(self_obj_type, st, func, ...) st.get(&self_obj_type::func, __VA_ARGS__)
#define CALL(self_obj_type, st, func, ...) CALL_mem(self_obj_type, st, func, __VA_ARGS__)
#define CALL1(self_obj_type, name, ...) CALL(self_obj_type, name##_mem, name##_, __VA_ARGS__)
#define CALL1_OUTSIDE(obj, name, ...)                                                              \
  (obj).CALL(&decltype(obj), objname##_mem, name##_, __VA_ARGS__)
template <typename T> size_t do_hash(const T &tt) { return std::hash<T>{}(tt); }
