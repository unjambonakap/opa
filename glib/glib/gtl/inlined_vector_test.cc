...
#include "glib/gtl/inlined_vector.h"
...
typedef glib::gtl::InlinedVector<int, 8> IntVec;
...
typedef glib::gtl::InlinedVector<Instance, 8> InstanceVec;
...
typedef glib::gtl::InlinedVector<RefCounted, 8> RefCountedVec;
...
typedef glib::gtl::InlinedVector<Dynamic, 8> DynamicVec;
...
TEST(InlinedVectorTest, NoDefaultCtor) {
  glib::gtl::InlinedVector<NoDefaultCtor, 1> v(10, NoDefaultCtor(2));
  (void)v;
}
TEST(InlinedVectorTest, NoCopy) {
  glib::gtl::InlinedVector<NoCopy, 1> v(10);
  (void)v;
}
TEST(InlinedVectorTest, NoAssign) {
  glib::gtl::InlinedVector<NoAssign, 1> v(10);
  (void)v;
}
...
TEST(IntVec, SelfRefPushBack) {
  ...
  glib::gtl::InlinedVector<string, 4> v;
...
TEST(RangedConstructor, SimpleType) {
  ...
  glib::gtl::InlinedVector<int, 4> v(source_v.begin(), source_v.end());
  glib::gtl::InlinedVector<int, 4> empty4;
  EXPECT_EQ(4, v.size());
  ...
  glib::gtl::InlinedVector<int, 2> realloc_v(source_v.begin(),
                                                   source_v.end());
  glib::gtl::InlinedVector<int, 2> empty2;
...
TEST(RangedConstructor, ComplexType) {
  ...
  // First try to fit in inline backing
  glib::gtl::InlinedVector<Instance, 1> v(source_v.begin(),
                                                source_v.end());
  glib::gtl::InlinedVector<Instance, 1> empty1;
  EXPECT_EQ(1, v.size());
  ...
  std::list<Instance> source_v2 = {Instance(0), Instance(1), Instance(2),
                                   Instance(3)};
  // Now, force a re-allocate
  glib::gtl::InlinedVector<Instance, 1> realloc_v(source_v2.begin(),
                                                        source_v2.end());
  EXPECT_EQ(4, realloc_v.size());
...
TEST(RangedConstructor, ElementsAreConstructed) {
  ...
  glib::gtl::InlinedVector<string, 1> v(source_v.begin(), source_v.end());
  EXPECT_EQ("cat", v[0]);
  EXPECT_EQ("dog", v[1]);
}

TEST(InitializerListConstructor, SimpleTypeWithInlineBacking) {
  auto vec = glib::gtl::InlinedVector<int, 3>{4, 5, 6};
...
TEST(InitializerListConstructor, SimpleTypeWithReallocationRequired) {
  auto vec = glib::gtl::InlinedVector<int, 2>{4, 5, 6};
  EXPECT_EQ(3, vec.size());
...
TEST(InitializerListConstructor, DisparateTypesInList) {
  EXPECT_EQ((std::vector<int>{-7, 8}),
            Vec(glib::gtl::InlinedVector<int, 2>{-7, 8ULL}));

  EXPECT_EQ(
      (std::vector<string>{"foo", "bar"}),
      Vec(glib::gtl::InlinedVector<string, 2>{"foo", string("bar")}));
}

TEST(InitializerListConstructor, ComplexTypeWithInlineBacking) {
  glib::gtl::InlinedVector<Instance, 1> empty;
  auto vec = glib::gtl::InlinedVector<Instance, 1>{Instance(0)};
...
TEST(InitializerListConstructor, ComplexTypeWithReallocationRequired) {
  auto vec =
      glib::gtl::InlinedVector<Instance, 1>{Instance(0), Instance(1)};
...
static void BM_InlinedVectorTenAssignments(int iters, int len) {
  typedef glib::gtl::InlinedVector<Buffer, 2> BufferVec;
...
static void BM_CreateFromInitializerList(int iters) {
  for (; iters > 0; iters--) {
    glib::gtl::InlinedVector<int, 4> x{1, 2, 3};
    (void)x[0];
  }
}
...
static void BM_LargeSwappableElements(int iters, int len) {
  typedef glib::gtl::InlinedVector<LargeSwappable, 32> Vec;
  Vec a(len);
  Vec b;
  while (--iters >= 0) {
    using std::swap;
