...
#include <iosfwd>
#include <string>
#include "glib/core/error_codes.pb.h"
...
class Status {
 public:
  ...
  Status(glib::error::Code code, glib::StringPiece msg);
  ...
  glib::error::Code code() const {
    return ok() ? glib::error::OK : state_->code;
  }
...
#define TF_CHECK_OK(val) CHECK_EQ(::glib::Status::OK(), (val))
#define TF_QCHECK_OK(val) QCHECK_EQ(::glib::Status::OK(), (val))
...
#define TF_DCHECK_OK(val) \
  while (false && (::glib::Status::OK() == (val))) LOG(FATAL)
#endif
