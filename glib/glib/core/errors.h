...
void AppendToMessage(::glib::Status* status, Args... args) {
  *status = ::glib::Status(
      status->code(),
      ::glib::strings::StrCat(status->error_message(), "\n\t", args...));
}

// For propagating errors when calling a function.
#define TF_RETURN_IF_ERROR(expr)                         \
  do {                                                   \
    const ::glib::Status _status = (expr);         \
    if (TF_PREDICT_FALSE(!_status.ok())) return _status; \
  } while (0)

#define TF_RETURN_WITH_CONTEXT_IF_ERROR(expr, ...)                  \
  do {                                                              \
    ::glib::Status _status = (expr);                          \
    if (TF_PREDICT_FALSE(!_status.ok())) {                          \
      ::glib::errors::AppendToMessage(&_status, __VA_ARGS__); \
      return _status;                                               \
    }                                                               \
  } while (0)
...
#define DECLARE_ERROR(FUNC, CONST)                                       \
  template <typename... Args>                                            \
  ::glib::Status FUNC(Args... args) {                              \
    return ::glib::Status(::glib::error::CONST,              \
                                ::glib::strings::StrCat(args...)); \
  }                                                                      \
  inline bool Is##FUNC(const ::glib::Status& status) {             \
    return status.code() == ::glib::error::CONST;                  \
  }
