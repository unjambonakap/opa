...
// Macros for testing the results of functions that return glib::Status.
#define TF_EXPECT_OK(statement) \
  EXPECT_EQ(::glib::Status::OK(), (statement))
#define TF_ASSERT_OK(statement) \
  ASSERT_EQ(::glib::Status::OK(), (statement))
...
// EXPECT_EQ(..expected glib::error::Code..., status.code());
