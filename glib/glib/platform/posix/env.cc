...
#include "glib/platform/load_library.h"
...
class PosixEnv : public Env {
 public:
  ...
  Status LoadLibrary(const char* library_filename, void** handle) override {
    return glib::internal::LoadLibrary(library_filename, handle);
  }

  Status GetSymbolFromLibrary(void* handle, const char* symbol_name,
                              void** symbol) override {
    return glib::internal::GetSymbolFromLibrary(handle, symbol_name,
                                                      symbol);
  }

  string FormatLibraryFileName(const string& name,
                               const string& version) override {
    return glib::internal::FormatLibraryFileName(name, version);
  }
};
