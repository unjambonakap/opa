...
#include "glib/core/status_test_util.h"
...
class InterPlanetaryFileSystem : public NullFileSystem {
 public:
  Status FileExists(const string& fname) override {
    ...
    return Status(glib::error::NOT_FOUND, "File does not exist");
  }
  ...
  Status CreateDir(const string& dirname) override {
    ...
    if (split_path.size() > 3) {
      return Status(glib::error::INVALID_ARGUMENT, "Bad dirname");
    }
    if (split_path.empty()) {
      return Status::OK();
    }
    ...
    if (split_path.size() == 2) {
      if (!BodyExists(split_path[0])) {
        return Status(glib::error::FAILED_PRECONDITION,
    ...
    return Status(glib::error::FAILED_PRECONDITION, "Failed to create");
  }
  ...
  Status IsDirectory(const string& dirname) override {
    ...
    std::vector<string> split_path = str_util::Split(parsed_path, '/');
    if (split_path.size() > 2) {
      return Status(glib::error::FAILED_PRECONDITION, "Not a dir");
    }
    if (celestial_bodies_.find(parsed_path) != celestial_bodies_.end()) {
      return Status::OK();
    }
